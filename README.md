...
we can employ authorisation through limiting of user access, so that entities only have access to their own data and authentication through login/password and multifactor-authentication, so that entities logging in are who they claim to be.
